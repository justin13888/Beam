 ...
 */
public fun BeamException.toFailure(): BeamFailure = when (this) {
    is BeamException.NoActiveServer -> BeamFailure(
        message = "No server selected. Add a Beam server to get started.",
        retryable = false,
    )

    is BeamException.UnknownServer -> BeamFailure(
        message = "That server is no longer set up on this device.",
        retryable = false,
    )

    is BeamException.InvalidServerUrl -> BeamFailure(
        message = "That address does not look like a Beam server.",
        retryable = false,
    )

    is BeamException.Unauthenticated -> BeamFailure(
        message = "Sign in to continue.",
        retryable = false,
        requiresSignIn = true,
    )

    is BeamException.SessionExpired -> BeamFailure(
        message = "Your session expired. Sign in again to continue.",
        retryable = false,
        requiresSignIn = true,
    )

    is BeamException.Forbidden -> BeamFailure(
    ...
    is BeamException.NotFound -> BeamFailure(message = detail, retryable = false)

    is BeamException.BadRequest -> BeamFailure(message = detail, retryable = false)

    is BeamException.RateLimited -> BeamFailure(
        message = "The server is busy. Try again in ${retryAfterSecs} seconds.",
        retryable = true,
    )

    is BeamException.Server -> BeamFailure(
    ...
    is BeamException.Network -> BeamFailure(
        message = "Could not reach the server. Check your connection.",
        retryable = retryable,
    )

    is BeamException.UntrustedCertificate -> BeamFailure(
        message = "This server's security certificate is not trusted.",
        retryable = false,
    )

    is BeamException.Protocol -> BeamFailure(
        message = "The server replied in a way this app did not understand.",
        retryable = false,
    )

    is BeamException.Storage -> BeamFailure(
