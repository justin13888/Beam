...
that glob and running `mise run ts:check`; the exclusion can go when that comes back clean.
`astro check` (`bun run typecheck`) validates content; there is no separate test suite, and
`mise run docs:build` is the only gate that meaningfully exercises the site. Deployment is
`astro build && wrangler pages deploy`, run from `release.yml` on a release tag; both that job and
CI's check out full git history because
