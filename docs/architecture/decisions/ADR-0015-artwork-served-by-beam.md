...
- Beam is now on the path of every image render, so an artwork bug degrades a grid where previously
  ...
  placeholder for it; the distinction exists for the operator, who can then tell a dead CDN from a
  title with no art. A stored URL Beam itself refuses to fetch is a `500`, since no provider was
  asked anything and the bad data is Beam's own.
