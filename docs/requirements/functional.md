...
SHOULD, SHOULD NOT, MAY) to indicate normative strength. Each requirement is intended to be
independently testable. See `product.md` for narrative context and `non-functional.md` for
...
- **FR-101**: The server MUST support authentication exclusively via OpenID Connect (OIDC). No
  password-based login, registration flow, or "forgot password" flow MUST exist.
...
- **FR-104**: Session state MUST be persisted server-side in Postgres. The server MUST NOT rely on
  Redis or any other session store for this push.
...
- **FR-110**: For local development, the server's identity provider MUST be satisfiable by Dex running
  via `compose.dependencies.yaml`, configured with static test users, requiring no external network
  dependency.

## FR-2xx — Library & Indexing

- **FR-201**: The server MUST run as a single binary (`beam-server`) that performs library indexing
  in-process. No separate indexer process or gRPC service boundary MUST be required at runtime.
...
- **FR-203**: The server MUST maintain a separate, writable data directory distinct from any library
  root, used for its own state (e.g., SQLite metadata cache, application cache).
- **FR-204**: The server MUST support classifying indexed filesystem entries into movies and
  TV shows/seasons/episodes, persisting them as `movie_entries` and `episode`-family rows
  respectively.
...
- **FR-207**: The server MUST support triggering a library scan from an admin action in the web
  client. Triggering a scan via manual out-of-band process invocation MUST NOT be required.
...
- **FR-301**: The server MUST enrich indexed movies and shows with metadata (poster URL, backdrop
  URL, description, ratings, genres, release date) via the `cameo` crate (v0.2.0), which unifies TMDB
  and AniList sources.
...
- **FR-305**: The server MUST support enriching TV show titles at the season and episode level
  (season/episode titles, descriptions, air dates), not only at the top-level show.
- **FR-306**: The server MUST allow enrichment to proceed for AniList-sourced titles without requiring
  any API key. The absence of a configured TMDB API key MUST NOT prevent AniList-sourced titles from
  being enriched.
- **FR-307**: If no TMDB API key is configured, the server MUST leave TMDB-eligible titles
...
- **FR-308**: The server MUST expose an admin-triggerable "re-enrich" action, scoped to a single title
  or to all titles, that re-runs enrichment regardless of current status.
...
- **FR-310**: Poster and backdrop images MUST be served to the client as direct URLs to the
  TMDB/AniList CDN. The server MUST NOT proxy, cache, or re-host these images in this push (see
  NFR-501).
...
- **FR-503**: The server MUST NOT generate or serve HLS/DASH manifests or segments. Any existing
  HLS-generation code path MUST be removed rather than completed.
...
- **FR-505**: For a title with multiple indexed file versions, the server MUST expose an endpoint
  enumerating the available versions (e.g., resolution, container, size) so the client can present a
...
- **FR-603**: The server MUST provide an admin-only endpoint to trigger metadata re-enrichment (FR-308),
  gated by admin role.
...
- **FR-702**: The web client MUST periodically report playback position to the server during active
  ...
  crash without requiring an explicit "save" action from the user.
